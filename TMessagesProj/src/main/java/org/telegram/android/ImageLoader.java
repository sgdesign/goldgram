...
import org.goldgram.messenger.DispatchQueue;
...
public class ImageLoader {
    ...
    private File گلد گرامPath = null;
        ...
        FileLoader.getInstance().setDelegate(new FileLoader.FileLoaderDelegate() {
            ...
            public void fileDidLoaded(final String location, final File finalFile, final int type) {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        if (location != null) {
                            if (MediaController.getInstance().canSaveToGallery() && گلد گرامPath != null && finalFile != null && finalFile.exists() && (location.endsWith(".mp4") || location.endsWith(".jpg"))) {
                                if (finalFile.toString().startsWith(گلد گرامPath.toString())) {
    ...
    private HashMap<Integer, File> createMediaPaths() {
        ...
        try {
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                گلد گرامPath = new File(Environment.getExternalStorageDirectory(), "گلد گرام");
                گلد گرامPath.mkdirs();
                ...
                try {
                    for (int a = 0; a < 5; a++) {
                        ...
                        File dstFile = new File(گلد گرامPath, "temp.file");
                ...
                if (canRename) {
                    if (گلد گرامPath.isDirectory()) {
                        try {
                            File imagePath = new File(گلد گرامPath, "گلد گرام Images");
                            imagePath.mkdir();
                        ...
                        try {
                            File videoPath = new File(گلد گرامPath, "گلد گرام Video");
                            videoPath.mkdir();
                        ...
                        try {
                            File audioPath = new File(گلد گرامPath, "گلد گرام Audio");
                            audioPath.mkdir();
                        ...
                        try {
                            File documentPath = new File(گلد گرامPath, "گلد گرام Documents");
                            documentPath.mkdir();
